import type { Collection, Gif, PublicUserProfile } from "../types";
              ...
              {selectedUserProfile.publicFavourites ? (
                selectedUserFavourites.length > 0 ? (
                  <div className="masonry-grid">
                    {selectedUserFavourites.map((gif, index) => (
                      <div key={gif.id} className="masonry-item rounded-2xl overflow-hidden bg-zinc-900 border border-white/5">
                        <img src={gif.images.fixed_height.url} alt={gif.title} className="w-full block" />
                        <div className="p-3">
                          <p className="text-sm text-white line-clamp-2">{gif.title || `GIF ${index + 1}`}</p>
                          <p className="text-xs text-zinc-500 mt-1">@{gif.username || "unknown"}</p>
                        </div>
                      </div>
                    ))}
                  </div>
