import type { Collection, Page, ProfileSettings, Workspace } from "../types";
    ...
    <div className="grid xl:grid-cols-2 gap-6">
      <SectionCard title="Synced Creator Toolbox" subtitle="Helper mode, public collections, and persistent analytics signals.">
        <div className="grid md:grid-cols-2 gap-4">
          <div className="panel-muted">Public collections: {publicCollections.length}</div>
          <div className="panel-muted">Recent history items: {workspace.history.length}</div>
          <div className="panel-muted">Imported GIFs: {analytics.importedCount}</div>
          <div className="panel-muted">Queue size: {analytics.queued}</div>
        </div>
        <div className="mt-4 flex items-center gap-3 flex-wrap">
          <button
            onClick={() => updateProfileField({ ...workspace.profile, helperMode: !workspace.profile.helperMode })}
            className="primary-btn"
          >
            {workspace.profile.helperMode ? "Disable" : "Enable"} helper mode
          </button>
          <button onClick={() => setPage("favourites")} className="secondary-btn">
            Open library
          </button>
        </div>
      </SectionCard>
