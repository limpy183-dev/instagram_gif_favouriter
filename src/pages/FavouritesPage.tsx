    ...
    <SectionCard title="Favourite Library" subtitle="Collections, tags, smart filters, and queue all sync through Supabase." action={<button onClick={handleClearAll} className="secondary-btn">Clear all</button>}>
      <div className="grid lg:grid-cols-[1fr_18rem] gap-6">
        <div className="space-y-4">
          ...
          {filteredFavourites.length > 0 ? <div className="masonry-grid">{filteredFavourites.map((gif, index) => <div key={gif.id}><GifCard gif={gif} index={index} onSelect={addHistory} isFavourited={true} onToggleFavourite={handleToggleFavourite} notePreview={workspace.gifMeta[gif.id]?.notes} isQueued={isQueued(gif.id)} onQueueToggle={handleQueueToggle} /><div className="mt-2 flex flex-wrap gap-2">{workspace.collections.filter((collection) => !['all-favourites', 'queue'].includes(collection.id)).map((collection) => <button key={collection.id} onClick={() => addGifToCollection(gif, collection.id)} className="chip">Add to {collection.name}</button>)}</div></div>)}</div> : <div className="empty-state"><div className="text-5xl">💔</div><p className="text-zinc-400 mt-3">No favourites match the current filters.</p></div>}
        </div>
        <div className="space-y-4">
