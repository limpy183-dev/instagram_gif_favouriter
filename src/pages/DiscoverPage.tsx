...
export function DiscoverPage({
  ...
  return (
    <div className="grid xl:grid-cols-[1.2fr_0.8fr] gap-6">
      <SectionCard
        title={currentLabel}
        subtitle="Search, trend surf, and save quickly for replies, stories, and reels."
        action={<span className="text-zinc-500 text-xs">{gifs.length} loaded</span>}
      >
        <div className="grid md:grid-cols-3 gap-3 mb-4">
          <div className="stat-card">
            <strong>{analytics.totalSaved}</strong>
            <span>Synced favourites</span>
          </div>
          <div className="stat-card">
            <strong>{analytics.queued}</strong>
            <span>Use later queue</span>
          </div>
          <div className="stat-card">
            <strong>{analytics.importedCount}</strong>
          ...
          <div className="masonry-grid">
            {gifs.map((gif, index) => (
              <GifCard
                ...
                index={index}
                onSelect={addHistory}
                isFavourited={isFavourited(gif.id)}
        ...
        {!loading && gifs.length === 0 && (
          <div className="empty-state">
            ...
            <h3 className="text-xl font-bold text-zinc-300 mb-2">No GIFs Found</h3>
            <p className="text-zinc-500 text-sm max-w-xs">Try a different search term or browse trending GIFs.</p>
          </div>
        )}
        {hasMore && !loading && (
          <div className="flex justify-center mt-8">
            <button
              onClick={() => fetchGifs(searchQuery, activeCategory, offset)}
              disabled={loadingMore}
              className="primary-btn"
            >
              {loadingMore ? "Loading more..." : "Load more GIFs"}
            </button>
          </div>
        )}
      </SectionCard>
        ...
        <SectionCard title="Creator Toolbox" subtitle="Synced helper settings, public collections, and offline fallback.">
          <div className="space-y-3 text-sm text-zinc-300">
            <div className="panel-muted">
              Multi-device sync: profile, collections, metadata, and history are stored in Supabase.
            </div>
        ...
        <SectionCard title="Recently Viewed" subtitle="Synced viewing history from Supabase.">
          <div className="grid grid-cols-2 gap-3">
            {recentHistory.length === 0 ? (
              <p className="text-zinc-500 text-sm">No recent views yet.</p>
            ) : (
              recentHistory.map((gif) => (
                <button key={gif.id} onClick={() => addHistory(gif)} className="recent-card">
                  <img src={gif.images.fixed_height.url} alt={gif.title} className="w-full h-24 object-cover rounded-xl mb-2" />
                  <span className="text-xs text-zinc-300 line-clamp-2">{gif.title}</span>
                </button>
              ))
            )}
          </div>
        ...
        <SectionCard title="External Import" subtitle="Imported GIFs are now persisted as metadata and shared collection items.">
          <div className="space-y-3">
            <input
              ...
              onChange={(e) => setManualImportTitle(e.target.value)}
            ...
            <input
              ...
              className="field"
            />
            <button onClick={importExternalGif} className="primary-btn w-full">
              Import external GIF
            </button>
          </div>
        </SectionCard>
