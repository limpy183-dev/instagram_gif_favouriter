...
export function Toast({ message, type, visible }: ToastProps) {
  ...
  return <div className={`fixed bottom-6 right-6 z-[100] flex items-center gap-3 px-5 py-3.5 rounded-2xl text-white text-sm font-semibold shadow-2xl ${styles[type]} toast-enter`} style={type === 'heart' ? { background: 'var(--accent)' } : undefined}>{type === 'success' && <CheckIcon />}{type === 'heart' && <HeartIcon filled />}{message}</div>;
}
  ...
  return (
    <div className="masonry-item gif-card relative rounded-2xl overflow-hidden cursor-pointer group" style={{ animationDelay: `${(Math.floor(index / 2) % 6) * 40}ms` }}>
      ...
      <div className="absolute top-2 right-2 flex gap-2 z-10">
        <button onClick={(e) => { e.stopPropagation(); onQueueToggle(gif); }} className={`mini-action ${isQueued ? 'text-white' : 'bg-black/50 text-white/70'}`} style={isQueued ? { background: 'var(--accent)' } : undefined} aria-label={isQueued ? 'Remove from queue' : 'Add to queue'}>{isQueued ? 'Q' : '+'}</button>
        <button onClick={(e) => { e.stopPropagation(); onToggleFavourite(gif); }} className={`mini-action ${isFavourited ? 'text-white' : 'bg-black/50 text-white/70 hover:text-white'}`} style={isFavourited ? { background: 'var(--accent)' } : undefined} aria-label={isFavourited ? 'Remove favourite' : 'Add favourite'}><HeartIcon filled={isFavourited} /></button>
      </div>
    </div>
  );
}
  ...
  return (
    <div ref={backdropRef} onClick={(e) => { if (e.target === backdropRef.current) onClose(); }} className="fixed inset-0 z-50 flex items-center justify-center p-4 modal-backdrop bg-black/75">
      <div className="relative bg-[var(--bg-elevated)] rounded-3xl overflow-hidden shadow-2xl shadow-black/60 max-w-3xl w-full border border-[var(--border-strong)] scale-in max-h-[92vh] overflow-y-auto">
        <button onClick={onClose} className="absolute top-4 right-4 z-10 bg-black/50 hover:bg-black/80 text-white rounded-full p-2 transition-colors backdrop-blur-md" aria-label="Close"><CloseIcon /></button>
        <button onClick={() => onToggleFavourite(gif)} className={`heart-btn absolute top-4 left-4 z-10 rounded-full p-2.5 backdrop-blur-md ${isFavourited ? 'text-white' : 'bg-black/50 text-white/70 hover:text-white'}`} style={isFavourited ? { background: 'var(--accent)' } : undefined} aria-label="Toggle favourite"><HeartIcon filled={isFavourited} /></button>
        <div className="grid lg:grid-cols-[1.1fr_0.9fr]">
          <div className="bg-[var(--bg)] flex items-center justify-center"><img src={gif.images.original.url} alt={gif.title} className="w-full max-h-[36rem] object-contain" /></div>
          <div className="p-5">
            <h3 className="text-white font-semibold text-base mb-3 tracking-tight text-balance">{gif.title || 'Untitled GIF'}</h3>
            <div className="rounded-xl px-4 py-3 mb-4 border border-[rgba(245,57,107,0.25)]" style={{ background: 'var(--accent-soft)' }}>
            ...
            <div className="mt-4">
              ...
              <div className="flex gap-2">
                <input value={draftTag} onChange={(e) => setDraftTag(e.target.value)} placeholder="Add tag" className="field flex-1" />
                <button onClick={() => { if (!draftTag.trim()) return; onAddTag(gif, draftTag); setDraftTag(''); }} className="primary-btn">Add</button>
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="max-w-7xl mx-auto px-4 py-4">
          <div className="flex items-center justify-between mb-4 gap-4 flex-wrap">
            <div className="flex items-center gap-3">
              <div className="w-9 h-9 rounded-xl flex items-center justify-center shadow-lg glow-pulse" style={{ background: 'var(--accent)', color: '#fff' }}><SparkleIcon /></div>
              <div>
          ...
          {page === 'discover' && <div className="flex flex-col gap-3 lg:flex-row lg:items-center"><div className="flex items-center gap-3 bg-[var(--bg-elevated)] border border-[var(--border)] rounded-2xl px-4 py-3 search-glow flex-1"><span className="text-[var(--text-muted)] flex-shrink-0"><SearchIcon /></span><input ref={inputRef} type="text" placeholder="Search millions of GIFs or describe a reaction..." value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} className="flex-1 bg-transparent text-white placeholder-[var(--text-faint)] text-sm font-medium outline-none" />{searchQuery && <button onClick={() => { setSearchQuery(''); inputRef.current?.focus(); }} className="text-[var(--text-muted)] hover:text-white transition-colors flex-shrink-0 hover:bg-white/10 p-1 rounded-full"><XIcon /></button>}{loading && <div className="w-4 h-4 border-2 rounded-full animate-spin flex-shrink-0" style={{ borderColor: 'var(--accent)', borderTopColor: 'transparent' }} />}</div><select value={moodFilter} onChange={(e) => setMoodFilter(e.target.value as MoodFilter)} className="bg-[var(--bg-elevated)] border border-[var(--border)] rounded-2xl px-4 py-3 text-sm text-white outline-none min-w-48 focus:border-[var(--accent)] transition-colors">{MOOD_PRESETS.map((mood) => <option key={mood.value} value={mood.value}>{mood.label}</option>)}</select></div>}
          {page === 'discover' && aiSuggestions.length > 0 && <div className="flex gap-2 overflow-x-auto mt-3 pb-1 no-scrollbar">{aiSuggestions.map((suggestion) => <button key={suggestion} onClick={() => setSearchQuery(suggestion)} className="chip">Try: {suggestion}</button>)}</div>}
          {page === 'discover' && <div className="flex gap-2 overflow-x-auto mt-3 pb-1 no-scrollbar">{CATEGORIES.map((category) => <button key={category.value} onClick={() => { setActiveCategory(category.value); setSearchQuery(''); setOffset(0); }} className={`flex-shrink-0 px-3.5 py-1.5 rounded-full text-xs font-semibold transition-all duration-300 hover:scale-105 ${activeCategory === category.value && !searchQuery ? 'pill-active' : 'bg-[var(--bg-elevated)] text-[var(--text-muted)] hover:text-white border border-[var(--border)] hover:border-[var(--border-strong)]'}`}>{category.label}</button>)}</div>}
        </div>
      </header>
